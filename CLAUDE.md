...
**Docs stay in sync with the data (`tools/sync_docs.py`).** The refresh-sensitive figures hand-cited in `README.md`/`CLAUDE.md` — institution counts and the gold-set F1 baseline — drift every quarter. `docs_stats.json` (committed, repo root) is the source of truth; `sync_docs.py` recomputes from the live snapshot and rewrites the docs: **counts** by replacing the prior comma-formatted token, the **F1 baseline** by regenerating the `SYNC:gold_readme` / `SYNC:gold_claude` HTML-comment marker spans. `scheduled_refresh.py` runs it (write mode) **only when `refresh_if_changed` reports `changed: True`**, isolated in a `try/except`. `sync_docs.py --check` reports drift without writing (post-refresh / manual). A hermetic test (`tests/test_doc_sync.py`) enforces that the docs and `docs_stats.json` agree, so a hand-edit to one without the other fails CI. **Code-driven figures use a different mechanism** (they change at commit time, not on a data refresh): the `11 tools` count is guarded by a hermetic test in `test_doc_sync.py` that fails CI if it drifts from `server.py`'s `@mcp.tool` count, so it's fixed in the same PR. The `39 fields` count (needs a built record to count) and the exact test count stay manual — keep test-count references vague ("60+ hermetic tests") rather than an exact integer that churns.
